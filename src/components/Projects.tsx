    ...
    <section id="projects" className="py-16 sm:py-32">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <AnimatedSection className="mb-9 grid gap-4 sm:mb-14 sm:gap-6 lg:grid-cols-[1fr_0.65fr] lg:items-end">
          ...
          <p className="text-sm leading-7 text-zinc-500">A closer look at the product context, engineering scope, and value behind each interface.</p>
        </AnimatedSection>
              ...
              <button type="button" onClick={() => setSelectedProject(project)} className="case-study group relative grid w-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a0a13] text-left lg:grid-cols-[0.9fr_1.1fr]">
                <div className="case-visual relative min-h-52 overflow-hidden border-b border-white/8 bg-[#080810] p-4 sm:min-h-72 sm:p-6 lg:border-b-0 lg:border-r">
                  ...
                  <div className="absolute left-5 top-5 z-10 flex items-center gap-2 rounded-full border border-white/10 bg-black/45 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.16em] text-zinc-400 backdrop-blur-sm">
                    <span className="status-pulse size-1.5 rounded-full bg-emerald-400" /> [ deployed / {project.year} ]
                  </div>
                  {project.imageUrl ? (
                    ...
                    <div className="relative flex h-full min-h-60 items-center justify-center"><span className="text-gradient text-7xl font-black">04</span></div>
                  )}
                ...
                <div className="relative flex flex-col p-5 sm:p-8">
                  <div className="mb-5 flex items-start justify-between gap-3 sm:mb-8 sm:gap-4">
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-indigo-300">{"// case-"}0{index + 1} · {project.role}</p>
                  ...
                  <div className="my-7 rounded-2xl border border-white/8 bg-white/[0.025] p-4">
                    <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-zinc-600">[ system-scope ]</p>
        ...
        {selectedProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md" onClick={() => setSelectedProject(null)}>
            <motion.article initial={{ opacity: 0, y: 24, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }} className="relative max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] border border-white/10 bg-[#0b0b14] p-6 sm:p-9" onClick={(event) => event.stopPropagation()}>
              ...
              <p className="section-kicker">case-study / {selectedProject.year}</p>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-5"><p className="font-mono text-[9px] uppercase tracking-[0.18em] text-zinc-600">My role</p><p className="mt-2 text-sm font-bold text-zinc-200">{selectedProject.role}</p><p className="mt-2 text-xs leading-6 text-zinc-500">{selectedProject.scope}</p></div>
                <div className="rounded-2xl border border-indigo-400/15 bg-indigo-400/[0.04] p-5"><p className="font-mono text-[9px] uppercase tracking-[0.18em] text-indigo-300">Outcome</p><p className="mt-2 text-xs leading-6 text-zinc-300">{selectedProject.outcome}</p></div>
              </div>
              <div className="mt-6 space-y-3">{selectedProject.highlights.map((item) => <div key={item} className="flex items-center gap-3 text-xs text-zinc-300"><span className="flex size-5 items-center justify-center rounded-full bg-emerald-400/10 text-emerald-300"><Check className="size-3" /></span>{item}</div>)}</div>
            </motion.article>
          </motion.div>
        )}
      </AnimatePresence>
